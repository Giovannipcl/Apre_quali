...
- Quantile regression models escape from the parametric context offering an approach that flexible the assumptions over the response variable.
- In quantile regression, each conditional quantile is modeled separately in terms of linear or smooth functions of explanatory variables.
- Based on the idea of quantile regression and on the independence of parametric imposes there is the Conditional Transformation Model (CTM).
- CTM addresses the direct estimation of the conditional distribution function of a random variable $Y$ conditional on a set of covariates $X = x$ `r Citep(myBib,"hothorn2014conditional")`.
...
- In this work we cover two distinct approaches for $h(Y|\boldsymbol{x})$ construction.
...
- We present an alternative Bayesian estimation procedure based on the Laplace approximation, motivated by the Integrated Nested Laplace Approximation (INLA) approach `r Citep(myBib,"rue2009approximate")`.
- The lower computational cost of the Laplace algorithm for posterior estimates compared to MCMC methods is a relevant aspect of the choice made.
...
- The transformation functions  $h(Y|\boldsymbol{x})$ can assume many forms.
...
___ 
- We have say that CTM is extremely dependent of the transformation function $h(Y|\boldsymbol{x})$ of the random variable $Y$. 
...
- There is several spline basis. Here we used B-splines `r Citep(myBib,"de1978practical")`.
...
---
- For the vector of unconstrained parameters $\boldsymbol{\beta}$, we set Gaussian prior distribution with precision matrix being the P-splines ﬁrst-order diﬀerence penalty starting with the second working parameter
...
- Multiple solutions of model coefficients may lead to the same approximated function of the spline space.
- As described by `r Citep(myBib,"wood2017generalized")` for unconstrained B-splines, to deal with this situation one may set a sum-to-zero constraint, that is, $\sum_{i = 1}^n f(x) = 0$.
- Let us consider the monotone B-spline in $y$ for a particular $j$ transformation

$$\begin{equation}\label{spline_f}
    h_j(y) = \sum_{l_1 = 1}^{L_1} B_{j,l_1,3,\boldsymbol{l}_1}(y) \gamma_{l_1} =  \boldsymbol{B}_j \boldsymbol{\gamma}^T =  \boldsymbol{B}_j \Sigma \boldsymbol{\tilde{\beta}}  =  \boldsymbol{X}_j \boldsymbol{\tilde{\beta}}.
\end{equation}$$


.content-box-gray[- Sum-to-zero restriction
    $$\boldsymbol{1}^T\boldsymbol{X}_j \boldsymbol{\tilde{\beta}} = 0, \forall \tilde{\boldsymbol{\beta}}$$
    $$\Rightarrow \boldsymbol{1}^T\boldsymbol{X}_j = \boldsymbol{0}$$
]

---

- A method to achieve this restriction is using QR decomposition of $(\boldsymbol{1}^T\boldsymbol{X}_j)^T$. Set as $\boldsymbol{Z}$ the final $L_1 - 1$ columns of the orthogonal matrix from the QR decompostion of $(\boldsymbol{1}^T\boldsymbol{X}_j)^T$, then we rewrite $\tilde{\boldsymbol{\beta}}$ as 

$$\begin{equation}
     \tilde{\boldsymbol{\beta}} = \boldsymbol{Z} \tilde{\boldsymbol{\beta}}'
 \end{equation}$$
 
- This construction will satisfy the constraint $\boldsymbol{1}^T\boldsymbol{X}_j \boldsymbol{\tilde{\beta}} = 0$ for any value of the $L_1 - 1$ dimensional vector $\tilde{\boldsymbol{\beta}}'$. 
- In practice, we post multiply $\boldsymbol{X}_j$ by $\boldsymbol{Z}$ and set the first element of $\tilde{\boldsymbol{\beta}}$ to zero. 
- The prior precision matrix for $\tilde{\boldsymbol{\beta}}'$ is the $\boldsymbol{K}_1$ with the first row and column removed.
- The tensor construction, $h(y|\boldsymbol{x})$ which involves $L_1 \times L_2$ parameters has the constraint

$$\begin{equation}\label{sumtozero}
    \sum_{q = 1}^{L_2} \beta_{j1q} = 0
\end{equation}$$


---

-  We achieve (\ref{sumtozero}) by introducing a re-parametrization matrix $\boldsymbol{Z}$, such that $\tilde{\boldsymbol{\beta}}_j = \boldsymbol{Z}\tilde{\boldsymbol{\beta}}_j'$, which

$$\begin{equation} \boldsymbol{Z} = \left(
    \begin{array}{c|c}
...
- We are interested in the marginal posterior distributions $\pi(\gamma_i|\boldsymbol{y})$
- The Laplace's method can be used to approximate these marginal densities.
---
- The marginal of $\gamma_i$ is given by

$$\begin{equation}\label{marginal_laplace}
    \pi(\gamma_i|\boldsymbol{y}) = \frac{\pi(\gamma_1,y)}{\pi(y)} = \frac{\int \pi(\boldsymbol{y}|\boldsymbol{\gamma}) \pi(\boldsymbol{\gamma}) d \boldsymbol{\gamma}_{-i}}{\int  \pi(\boldsymbol{y}|\boldsymbol{\gamma})\pi(\boldsymbol{\gamma}) d \boldsymbol{\gamma}}
     = \frac{\int \pi(\boldsymbol{\gamma})e^{g(y;\boldsymbol{\gamma})} d \boldsymbol{\gamma}_{-i} }{\int \pi(\boldsymbol{\gamma})e^{g(y;\boldsymbol{\gamma})} d \boldsymbol{\gamma}}
\end{equation}$$
- ${\gamma}_{-i}$ is the vector of parameters removing the ith parameter, $g(y;\boldsymbol{\gamma})$ is the log-likelihood of the model and $\pi(\boldsymbol{\gamma}) = \pi(\boldsymbol{\gamma}_s) \pi(\boldsymbol{\gamma}_u)$. 
- To obtain an approximation for $\pi(\gamma_i|\boldsymbol{y})$ is necessary to apply the Laplace approximation in numerator and denominator in (\ref{marginal_laplace}).
...
- The posterior mode $\boldsymbol{\gamma_0}$ of $\pi(\boldsymbol{\gamma}|\boldsymbol{y}) = \pi(\boldsymbol{\gamma})e^{g(y;\boldsymbol{\gamma})}$ is find by Newton-Raphson method.
...
---
.pull-left[
```{r, echo=FALSE, fig.height=10, fig.width=10, dpi = 300 , fig.cap="Approximated distribution for one hyperparameter.", fig.retina = 2,fig.align='center'}
...
.pull-right[
```{r, echo=FALSE, fig.height=10, fig.width=10, dpi = 300 , fig.cap="Approximated distribution for two hyperparameter.", fig.retina = 2,fig.align='center'}
...
- Using the approximated distributions obtained it is complicated to obtain the resultanting distribution of the combination.
- An alternative is to use simulated samples from the marginal posterior distribution of parameters and obtain an estimate of the conditional distribution. 
- Simulations from these marginals are possible by the Metropolis-Hasting (MH) algorithm.
...
$$\begin{equation}
 ...
 \end{equation}$$
- The posterior mean estimate for $F_{Y|\boldsymbol{X} = \boldsymbol{x}}(y)$ is 
...
- We discuss information criteria in the context of our presented models.
- For our context, we used them to contrast models with different transformation functions $h$, for example, models with different numbers of B-splines parameters.
- The first criteria is the Deviance Information Criteria (DIC) proposed by `r Citep(myBib,"spiegelhalter2002bayesian")`. 
- The DIC is defined in terms of the deviance, $D(\boldsymbol{\beta}, \boldsymbol{\tau}) =-2\log p(y|\boldsymbol{\tau}, \boldsymbol{\beta})$ and a model complexity measure named as the effective number of parameters, $p_{DIC}$.

$$\begin{equation}
    \textrm{DIC} = -2 \log \pi(y| \hat{\boldsymbol{\beta}}, \hat{\boldsymbol{\tau}}) + 2p_{DIC},
\end{equation}$$

where, 
$$\begin{equation}\label{pdic}
    p_{DIC} = E_{\boldsymbol{\beta}, \boldsymbol{\tau}|\boldsymbol{y}}[D(\boldsymbol{\beta}, \boldsymbol{\tau})] - D(\hat{\boldsymbol{\beta}}, \hat{\boldsymbol{\tau}}),
\end{equation}$$
---

- The first term in $p_{DIC}$ is written as

$$\begin{equation}\label{pdic_int}
    E_{\boldsymbol{\beta}, \boldsymbol{\tau}|\boldsymbol{y}}[-2\log p(y|\boldsymbol{\tau}, \boldsymbol{\beta})] = \int \int -2\log p(y|\boldsymbol{\tau}, \boldsymbol{\beta}) p(\boldsymbol{\beta}|\boldsymbol{\tau}, \boldsymbol{y})p(\boldsymbol{\tau}|\boldsymbol{y})d\boldsymbol{\beta}d\boldsymbol{\tau}
\end{equation}$$
- The integral on $\boldsymbol{\beta}$ in (\ref{pdic_int}) is evaluated by Monte Carlo since $p(\boldsymbol{\beta}|\boldsymbol{\tau}, \boldsymbol{y})$ is roughly Gaussian. 
- We approximate the integral of $p(\boldsymbol{\tau}|\boldsymbol{y})$ by a trapezoidal rule. 
- The $D(\hat{\boldsymbol{\beta}}, \hat{\boldsymbol{\tau}})$ is just a plug-in estimate using the posterior mean estimate of $\boldsymbol{\beta}$ and the posterior mode of $\boldsymbol{\tau|\boldsymbol{y}}$.
- The WAIC criterion `r Citep(myBib,"watanabe2010asymptotic")` , unlike DIC, is not based on the deviance, but in the Bayesian predictive distribution $p^*(y)$

$$\begin{equation}
    \textrm{WAIC} = -2\left(\sum_i \log p^*(y_i) - p_{WAIC}\right).
\end{equation}$$
with 

$$\begin{equation}
    p_{WAIC} = \sum_i \lbrace E_{\boldsymbol{\beta}, \boldsymbol{\tau}|\boldsymbol{y}}[(\log p(y_i|\boldsymbol{\tau}, \boldsymbol{\beta}))^2] - E_{\boldsymbol{\beta}, \boldsymbol{\tau}|\boldsymbol{y}}[\log p(y_i|\boldsymbol{\tau}, \boldsymbol{\beta})]^2 \rbrace.
\end{equation}$$
---
### Applications
___
...
knitr::include_graphics("figuras/dens_geys.png")
```
...
knitr::include_graphics("figuras/dens_gey_HPD.png")
```
]
...
p <- ggplot(db, aes(x = age, y = head)) +
  ...
  xlab("Age") + ylab("Head Circumference")+theme(axis.text=element_text(size=18),
...
```{r, echo=FALSE, fig.height=5, fig.width=12, dpi = 300 , fig.retina = 2,fig.align='center'}
...
where $\boldsymbol{a}$ and $\boldsymbol{b}(age)$ are vectors of evaluated cubic B-spline basis of dimension 7. The vector $\boldsymbol{\gamma} = (\gamma_1, \dots, \gamma_{49})$ is the basis coefficients model. 
...
.pull-left[
```{r, echo=FALSE, out.width = "80%", dpi = 300 , fig.cap="Coefficients from the VCM model.", fig.retina = 2,fig.align='center'}
knitr::include_graphics("figuras/coefs_vcm.png")
```

```{r, echo=FALSE, out.width = "80%", dpi = 300 , fig.cap="Coefficients from the VCM random effects model.", fig.retina = 2,fig.align='center'}
knitr::include_graphics("figuras/coefs_vcm_re.png")
```
 
]
.pull-right[
```{r, echo=FALSE, out.width = "80%", dpi = 300 , fig.cap="Coefficients from the Tensor model.", fig.retina = 2,fig.align='center'}
knitr::include_graphics("figuras/coef_tensor.png")
```

```{r, echo=FALSE, out.width = "80%", dpi = 300 , fig.cap="Coefficients from the Tensor random effects model.", fig.retina = 2,fig.align='center'}
knitr::include_graphics("figuras/coef_tensor_re.png")
```
] 
...
.pull-left[
```{r, echo=FALSE, out.width = "90%", dpi = 300 , fig.cap="95% quantile interval and the median for every individual posterior cumulative distribution estimated by INLA based algorithm for the VCM random effects model.", fig.retina = 2,fig.align='center'}
knitr::include_graphics("figuras/Tensor_vcm_IC.png")
```

```{r, echo=FALSE, out.width = "90%", dpi = 300 , fig.cap="95% quantile interval and the median for every individual posterior cumulative distribution estimated by stochastic simulation for the VCM random effects model.", fig.retina = 2,fig.align='center'}
knitr::include_graphics("figuras/Tensor_vcm_IC_simu.png")
```
 
]
.pull-right[
```{r, echo=FALSE, out.width = "90%", dpi = 300 , fig.cap="95% quantile interval and the median for every individual posterior cumulative distribution estimated by INLA based algorithm for the tensor random effects model.", fig.retina = 2,fig.align='center'}
knitr::include_graphics("figuras/Tensor_re_IC.png")
```

```{r, echo=FALSE, out.width = "90%", dpi = 300 , fig.cap="95% quantile interval and the median for every individual posterior cumulative distribution estimated by stochastic simulation for the tensor random effects model.", fig.retina = 2,fig.align='center'}
...
knitr::include_graphics("figuras/chol_dist_age.png")
```
]
...
knitr::include_graphics("figuras/moments.png")
```
]
---
### Final remarks
...
- Some alterations in the BCTM models presented here are available for discussion.
  - he $F_Z$ specifying could include an alternative to Gaussian distribution.
